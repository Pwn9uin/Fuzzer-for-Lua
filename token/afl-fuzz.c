...
static void write_to_testcase(void* mem, u32 len) {
  ...
  //printf("[*] Before Decode mem: %s\n", mem);
  decode(&mem, &len);
  //printf("[*] After Decode mem: %s\n", mem);
...
static void show_stats(void) {
  ...
  sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
          ...
          cYEL "token level fuzzer", use_banner);
...
static u8 fuzz_one(char** argv) {
  ...
  //encode_file(queue_cur->fname, &queue_cur->len);
...
retry_splicing:
  ...
  if (use_splicing && splice_cycle++ < SPLICE_CYCLES &&
    ...
    encode_file(target->fname, &target->len);
    ...
    goto havoc_stage;
    //unsigned int ll = target->len;
    //decode_file(target->fname, &ll);
  }
